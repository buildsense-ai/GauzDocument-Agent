...
# === 报告指南模板表 ===
@dataclass
class ReportGuideTemplate:
    """报告指南模板"""
    guide_id: str                      # 指南ID
    document_type_id: str              # 关联的文档类型ID
    template_name: str                 # 模板名称
    project_category: str              # 适用项目类别（如文物保护、建筑评估等）
    target_objects: List[str]          # 适用对象（如医灵古庙、古建筑等）
    report_guide: Dict[str, Any]       # 报告指南JSON数据
...
def get_metadata_class(content_type: str):
    ...
    mapping = {
        ...
        "report_guide": ReportGuideTemplate,
    }
...
def create_report_guide_id(document_type: str, project_category: str, sequence: int = 1) -> str:
    """创建报告指南ID"""
    return f"guide_{document_type}_{project_category}_{sequence:03d}"

def match_report_guide_by_keywords(guide_list: List[ReportGuideTemplate], keywords: List[str]) -> List[ReportGuideTemplate]:
    """根据关键词匹配报告指南"""
    matched_guides = []
    for guide in guide_list:
        # 检查模板名称
        name_match = any(keyword.lower() in guide.template_name.lower() for keyword in keywords)
        # 检查项目类别
        category_match = any(keyword.lower() in guide.project_category.lower() for keyword in keywords)
        # 检查适用对象
        object_match = any(
            any(keyword.lower() in obj.lower() for keyword in keywords)
            for obj in guide.target_objects
        )
        
        if name_match or category_match or object_match:
